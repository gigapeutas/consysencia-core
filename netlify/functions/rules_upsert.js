export default async (req) => {
  try {
    ...
    const payload = {
      ...
      match_kind: String(body.match_kind || "contains"),
      match_text: String(body.match_text || "").trim(),
      match_group: body.match_group == null ? null : String(body.match_group || "").trim(),
      match_sender: body.match_sender == null ? null : String(body.match_sender || "").trim(),
      reply_text: String(body.reply_text || "").trim(),
      updated_at: new Date().toISOString(),
    };

    if (!payload.match_text) return json(400, { ok: false, error: "match_text_required" }, corsHeaders());
    if (!payload.reply_text) return json(400, { ok: false, error: "reply_text_required" }, corsHeaders());

    // upsert via REST: POST /reply_rules?on_conflict=id
    const qs = new URLSearchParams();
    qs.set("on_conflict", "id");
    ...
    return json(200, { ok: true, row: rows[0] || null }, corsHeaders());
  } catch (e) {
